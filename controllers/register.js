...
const addUserToUserList = require("../model/addUserToUserList")
...
const checkReg = data => {
    if (/注册/.test(data.Content[0])) {
        setUserState(data.FromUserName[0], "0")
...
const quit = data => (data.Content[0] === "d" || data.Content[1] === "D") && setUserState(data.FromUserName[0], "null")

const checkWechatId = data => {
    if (data.Content[0] === "a" || data.Content[0] === "A") {
        setUserState(data.FromUserName[0], "1")
        return "请输入您的微信ID"
    }
...
const changeWechatId = data => {
    ...
    if (testWechatId.test(data.Content[0])) {
        setUserWeChat(data.FromUserName[0], data.Content[0])
        setUserState(data.FromUserName[0], "2")
...
const checkUserName = data => {
    if (data.Content[0] === "b" || data.Content[0] === "B") {
        setUserState(data.FromUserName[0], "3")
        return "请输入您的昵称"
    }
...
const changeUserName = data => {
    setUserName(data.FromUserName[0], data.Content[0])
    setUserState(data.FromUserName[0], "4")
...
const checkUserInfo = (data, rData) => {
    if (data.Content[0] === "c" || data.Content[0] === "C") {
        setUserState(data.FromUserName[0], "5")
...
const changeUserInfo = (data, rData) => {
    if (data.Content[0] === "y" || data.Content[0] === "Y") {
        setUserReadyState(data.FromUserName[0], "true")
        addUserToUserList(data.FromUserName[0]);
        return "注册完成！"
...
const register = (data, rData) => {
    const userId = data.FromUserName[0]
    if (!rData) {
